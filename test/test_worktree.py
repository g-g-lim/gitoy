...
class TestWorktreeFindPaths:
    ...
    def test_find_paths_with_normal_file(self, worktree: Worktree, test_file: Path):
        result = worktree.find_paths(test_file.name, test_file.parent)
        assert len(result) == 1
        assert result[0].name == test_file.name
    ...
    def test_find_paths_with_file_in_directory(self, worktree: Worktree, test_directory: Path, test_root_directory: Path):
        ...
        result = worktree.find_paths('./' + test_directory.name, test_root_directory)
        ...
        assert len(result) == 2
        for path in result:
            assert path.name in [(test_directory / Path(path)).name, (test_directory / Path(path2)).name]
    ...
    def test_find_paths_with_subdirectory(self, worktree: Worktree, test_directory: Path, test_root_directory: Path):
        ...
        result = worktree.find_paths(test_directory.name + '/' + subdir.name + '/' + 'testfile', test_root_directory)
        assert len(result) == 1
        assert result[0].name == file_subdir.name
        ...
        result = worktree.find_paths(test_directory.name + '/' + subdir.name, test_root_directory)
        assert len(result) == 2
        for path in result:
            assert path.name in [file_subdir.name, file2_subdir.name]
    ...
    def test_file_paths_with_dot_path(self, worktree: Worktree, test_directory: Path, test_root_directory: Path):
        ...
        result = worktree.find_paths('.', test_directory)
        assert len(result) == 1
        assert result[0].is_file()
        assert result[0].name == (test_directory / Path(path)).name
