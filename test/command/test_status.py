...
class TestStatusCommand:
    ...
    def test_status_command_empty_repository(
        self, init_command: Init, status_command: Status
    ):
    ...
    def test_status_command_with_untracked_files(
        self, init_command: Init, status_command: Status, test_directory: Path
    ):
        ...
        with patch("sys.stdout", new_callable=StringIO) as mock_stdout:
            with patch("os.getcwd", return_value=test_directory.as_posix()):
                ...
                assert (
                    f"new file: {path.relative_to(test_directory.parent)}"
                    in mock_stdout.getvalue()
                )
                assert "unstaged" not in mock_stdout.getvalue()

    def test_status_command_with_unstaged_files(
        self,
        init_command: Init,
        ...
        with patch("sys.stdout", new_callable=StringIO) as mock_stdout:
            with patch("os.getcwd", return_value=test_directory.as_posix()):
                ...
                assert (
                    f"modified: {path.relative_to(test_directory.parent)}"
                    in mock_stdout.getvalue()
                )

    def test_status_command_mixed_file_states(
        self,
        init_command: Init,
        status_command: Status,
        repository: Repository,
        ...
        with patch("sys.stdout", new_callable=StringIO) as mock_stdout:
            with patch("os.getcwd", return_value=test_directory.as_posix()):
                ...
                assert "untracked" in mock_stdout.getvalue()
                assert (
                    f"modified: {modified_path.relative_to(test_directory.parent)}"
                    in mock_stdout.getvalue()
                )
                assert (
                    f"deleted: {deleted_path.relative_to(test_directory.parent)}"
                    in mock_stdout.getvalue()
                )
                assert (
                    f"new file: {untracked_path.relative_to(test_directory.parent)}"
                    in mock_stdout.getvalue()
                )

    def test_status_command_mixed_file_states_with_root_directory(
        self,
        init_command: Init,
        status_command: Status,
        repository: Repository,
    ...
    def test_status_command_mixed_file_states_with_root_directory_2(
        self,
        init_command: Init,
        status_command: Status,
        repository: Repository,
